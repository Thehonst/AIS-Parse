...
{   
    ...
    f.log<<"消息1,2,3,5由A类船只发出,消息18、19由B类船只发送,消息21由AtoN(助航设备：如灯塔、浮标、导航标志)发送,24A部分由AIS台发送(船只、基站、助航设备),24B部分由A类船只发送"<<endl;
    f.log<<"消息5的目的地通常是一个MMSI(海上移动业务标识)号码,这是一个由30位数字组成的唯一标识符,用于识别船舶、基站或其他海上实体。MMSI号码的格式遵循国际"<<endl;
    f.log<<"电信联盟(ITU)的规则,其中前3位是国家代码,后27位是船舶或其他实体的唯一标识。因此,消息5的目的地最终是一个由30位数字组成的MMSI号码,而不是一串数字。"<<endl;
    f.log<<"这个MMSI号码用于在AIS消息中标识特定的船舶或基站,确保消息能够被正确地发送到预定的目的地。"<<endl;
